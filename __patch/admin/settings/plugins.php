...
if ($hasconfig) {
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('mod') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'modsettings', $hasconfig);
    }
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('format') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'formatsettings', $hasconfig);
    }
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('block') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'blocksettings', $hasconfig);
    }
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('message') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'messageoutputs', $hasconfig);
    }
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('auth') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'authsettings', $hasconfig);
    }
    ...
    foreach(core_plugin_manager::instance()->get_plugins_of_type('enrol') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'enrolments', $hasconfig);
    }
...
/// Editor plugins
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('editor') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'editorsettings', $hasconfig);
    }
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('antivirus') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'antivirussettings', $hasconfig);
    }
...
/// Filter plugins
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('filter') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'filtersettings', $hasconfig);
    }
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('repository') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'repositorysettings', $hasconfig);
    }
...
/// Web services
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('webservice') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'webservicesettings', $hasconfig);
    }
...
if ($hasconfig || has_capability('moodle/question:config', $systemcontext)) {
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('qtype') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'qtypesettings', $hasconfig);
    }
}

// Plagiarism plugin settings
if ($hasconfig && !empty($CFG->enableplagiarism)) {
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('plagiarism') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'plagiarism', $hasconfig);
    }
}
    ...
    foreach (core_component::get_plugin_list('coursereport') as $report => $path) {
        ...
        if (file_exists($file)) {
            $settings = new admin_settingpage('coursereport' . $report,
                    new lang_string('pluginname', 'coursereport_' . $report), $capability);
...
if ($hassiteconfig) {
    ...
    foreach (core_component::get_plugin_list('cachestore') as $plugin => $path) {
        ...
        if (file_exists($settingspath)) {
            $settings = new admin_settingpage('cachestore_'.$plugin.'_settings', new lang_string('pluginname', 'cachestore_'.$plugin), $capability);
            include($settingspath);
...
if ($hassiteconfig) {
    ...
    foreach (core_plugin_manager::instance()->get_plugins_of_type('calendartype') as $plugin) {
        ...
        $plugin->load_settings($ADMIN, 'calendartype', $hasconfig);
    }
}
...
foreach (core_plugin_manager::instance()->get_plugins_of_type('local') as $plugin) {
    ...
    $plugin->load_settings($ADMIN, null, $hasconfig);
}
