...
if ($hasconfig || has_any_capability($capabilities, $systemcontext)) { // speedup for non-admins, add all caps used on this page
    ...
    // PATCH : Restrict theme settings for non site admins
    ...
    if ($hassiteconfig) {
        ...
        $temp->add(new admin_setting_heading('themelist', new lang_string('allowedthemes', 'local_adminsettings'), '<div class="setting-enabled-themes">'.$CFG->themelist.'</div>'));
    }
...
preferences,moodle|/user/preferences.php|preferences',
    ...
    $availablethemes = array($CFG->theme) + explode(',', @$CFG->themelist);
    foreach (core_component::get_plugin_list('theme') as $theme => $themedir) {
        // PATCH : Restrict theme settings for non site admins
        if (!in_array($theme, $availablethemes) && !$hassiteconfig) continue;
        // /PATCH
